...
Use `entire checkpoint search` and `entire checkpoint explain` to recall the closest prior session for a task and turn it into a playbook the user can act on. This is task-shaped, not list-shaped: the goal is "here's how to do your task" rather than "here are some checkpoints."
...
- Use only the canonical Entire commands for this skill: `entire checkpoint search` and `entire checkpoint explain`.
...
- Pass the user's task description (and any derived alternate phrasing) to `entire checkpoint search` as a single shell-quoted argument. Strip or escape embedded quotes, backticks, `$(...)`, and `;` before substituting into the command — never paste user text directly into a shell snippet.
...
2. Treat `entire checkpoint search` and `entire checkpoint explain` as authentication-gated. If either reports authentication is required, stop and tell the user:

`entire checkpoint search` requires authentication. Run `entire login` and try again.
...
```bash
entire checkpoint search "<original task phrasing>" --json --limit 15 --date month
entire checkpoint search "<alternate phrasing>" --json --limit 15 --date month
```
