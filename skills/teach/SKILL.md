...
- Pass any user-supplied topic or transcript-derived term to `entire checkpoint search` as a single shell-quoted argument. Strip or escape embedded quotes, backticks, `$(...)`, and `;` before substituting into the command — never paste user text directly into a shell snippet.
...
```bash
entire checkpoint search "<topic>" --json --limit 25 --date month
```
